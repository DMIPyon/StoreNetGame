...
exports.deleteGame = exports.updateGame = exports.createGame = void 0;
const database_1 = require("../db/database");
// Crear un nuevo juego
const createGame = async (req, res) => {
    const { title, price, discount = 0, description = '', cover_url = '', banner_url = '', developer = '', release_date = new Date().toISOString().split('T')[0], category_ids = [] } = req.body;
    if (!title || !price) {
    ...
    try {
        ...
        try {
            ...
            // Insertar el juego
            const gameResult = await client.query(`INSERT INTO games (title, price, discount, description, cover_url, banner_url, developer, release_date)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
         RETURNING *`, [title, price, discount, description, cover_url, banner_url, developer, release_date]);
            const game = gameResult.rows[0];
            // Asociar categorías si se proporcionaron
            if (category_ids.length > 0) {
                const gameId = game.id;
                // Crear consulta para insertar múltiples asociaciones game-category
                const values = category_ids.map((catId, index) => `($1, $${index + 2})`).join(', ');
                const params = [gameId, ...category_ids];
                await client.query(`INSERT INTO game_categories (game_id, category_id) 
           VALUES ${values}`, params);
            }
            // Commit de la transacción
            await client.query('COMMIT');
            // Obtener el juego con sus categorías
            const completeGameResult = await client.query(`SELECT g.*, array_agg(c.id) as category_ids, array_agg(c.name) as categories
         FROM games g
         LEFT JOIN game_categories gc ON g.id = gc.game_id
         LEFT JOIN categories c ON gc.category_id = c.id
         WHERE g.id = $1
         GROUP BY g.id`, [game.id]);
...
const updateGame = async (req, res) => {
    const { id } = req.params;
    const { title, price, discount, description, cover_url, banner_url, developer, release_date, category_ids } = req.body;
    try {
        const client = await database_1.pool.connect();
        try {
            // Comenzar transacción
    ...
    try {
        ...
        `);
                ...
                if (tableCheck.rows[0].exists) {
                    await client.query('DELETE FROM cart_items WHERE game_id = $1', [id]);
                }
            }
            catch (error) {
