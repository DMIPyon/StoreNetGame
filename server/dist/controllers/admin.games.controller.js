...
const createGame = async (req, res) => {
    try {
        const { name, description, price, discount, cover_url, banner_url, developer, publisher, release_date, categories, rating } = req.body;
        ...
        // Insertar el juego
        const gameResult = await database_1.pool.query(`INSERT INTO games (
      ...
      RETURNING id`, [
        ...
        if (categories && Array.isArray(categories) && categories.length > 0) {
            ...
            await database_1.pool.query(`INSERT INTO game_categories (game_id, category_id) VALUES ${categoryValues}`);
        }
        res.status(201).json({
            success: true,
...
const updateGame = async (req, res) => {
    try {
        ...
        const checkGame = await database_1.pool.query('SELECT id FROM games WHERE id = $1', [gameId]);
        ...
        if (categories && Array.isArray(categories)) {
            ...
            await database_1.pool.query('DELETE FROM game_categories WHERE game_id = $1', [gameId]);
...
const deleteGame = async (req, res) => {
    try {
        ...
        await database_1.pool.query('DELETE FROM game_categories WHERE game_id = $1', [gameId]);
        // Eliminar el juego
        await database_1.pool.query('DELETE FROM games WHERE id = $1', [gameId]);
        res.status(200).json({
            success: true,
